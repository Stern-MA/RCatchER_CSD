function [hfd,cfreq,ord]=hifi(d,si,cfreq,varargin)
% ** function [hfd,cfreq,ord]=hifi(d,si,cfreq,varargin)
%   Passes data d through butterworth highpass filter using double reverse 
...
try
  ...
  while ~finito
    ...
    wp=efreq/hfsample;		
    % stopband ends at half passband frequency
    ws=.5*wp;
    [ord,wn] = buttord(wp,ws,rp,rs);
    % deviation of current -3dB freq from targeted -1.5 dB freq in Hz (should be positive):
  ...
  if verbose
    ...
    for i=1:length(frange)-1
      rs_est(i)=log10(mag(i+1)/mag(i))/log2(frange(i+1)/frange(i));
    end
