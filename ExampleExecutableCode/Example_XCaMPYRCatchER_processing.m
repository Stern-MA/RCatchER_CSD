%EXAMPLE SCRIPT FOR PROCESSING XCaMP-Y-RCatchER PTZ DATA
...
%use Suite2P to extract calcium transients by channel manually removing
%noise ROIs
...
%Output is .mat file structure with fields to work with data for analysis

%Outline
%  0) Start loop over chosen recordings
%  1) Load recording parameters
%  2) Define Variables
%  3) Load EEG from EDF and annotation files (Pinnacle)
%  4) Load DC from .abf file
%  5) Load Ca data from .mat files (S2P output)
%  6) Partition multiple seizure recording (if necessary)
%  7) Sort the F values into true cells and generate sorting indicies
%  8) Subtract background signal and neuropil contamination
%  9) Generate mean traces
% 10) Determine dF/F
% 11) Filter dF/F data...used for interim ploting
% 12) Correct for EEG bias (time shift) between 2p and eeg (if necessary)
% 13) Power spectral density determinations
% 14) Death determination
% 15) Mean seizure and CSD seed time determination
% 16) Determine end time of the seizure
% 17) Find times for post-ictal stimulation (if necessary)
% 18) Mean pre-ictal spike seed time determination
% 19) EEG kernal generation and refinement of EEG PIS detection
% 20) First PIS determination and Final PIS SEED adjustment
% 21) Filtering of Individual Traces to 1 Hz ...used for individual cell detection
% 22) Individual trace seizure recruitment determination
% 23) Define if a cell is recruited during seizure
% 24) Individual trace CSD recruitment determination
% 25) End of CSD determination in all modalities
% 26) Determine post-ictal calcium level
% 27) Individual trace PIS recruitment determination ---RecTimeG_PIS.v3
% 28) Define if spike included in analysis (>10% cell rec) ---EEG_PIS_times3, recTimeG_PIS.true
% 29) Determine calcium level change during PIS
% 30) Optional plots of key recruitment times
% 31) Generate data structure
% 32) Save data structure
%
%
%
%
...
%time (smaller of the red or green channel)
if isnan(MeanSzRecTimeG)
...
%use the Yellow/cytosol Sz time seed the Red
...
%Use the Yellow CSD time seed the Red until red is fixed
